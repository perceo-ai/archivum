...
| MCP streamable HTTP | Verified | 2026-09-08: one ASGI app now serves `/sse` and `/mcp` on the MCP port, so clients that only speak streamable HTTP (Codex and other rmcp clients) no longer get 405 from `/sse`. Covered by `tests/mcp_tests/test_streamable_http.py`; local `python -m archivum.mcp.server --sse` returned 200 for an `initialize` POST on `/mcp`, 401 for both endpoints without a bearer. Not re-verified inside Docker. |
